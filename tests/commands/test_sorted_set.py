...
class TestSortedSet:
    ...
    async def test_zadd_options(self, client, _s):
        ...
        assert int(await client.zadd("a{foo}", {"a1": 2}, condition=PureToken.NX)) == 0
        assert int(await client.zadd("a{foo}", {"a1": 2}, condition=PureToken.NX, change=True)) == 0
        assert int(await client.zscore("a{foo}", "a1")) == 1
        assert await client.zcard("a{foo}") == 1
        assert int(await client.zadd("a{foo}", {"a2": 1}, condition=PureToken.XX)) == 0
        assert await client.zcard("a{foo}") == 1
        assert int(await client.zadd("a{foo}", {"a1": 2}, condition=PureToken.XX)) == 0
        assert int(await client.zadd("a{foo}", {"a1": 3}, condition=PureToken.XX, change=True)) == 1
        assert int(await client.zscore("a{foo}", "a1")) == 3
        assert int(await client.zadd("a{foo}", {"a2": 1}, condition=PureToken.NX)) == 1
        assert int(await client.zadd("a{foo}", {"a3": 1}, condition=PureToken.NX, change=True)) == 1
        assert await client.zcard("a{foo}") == 3
        await client.zadd("a{foo}", {"a3": 1}, increment=True)
        assert int(await client.zscore("a{foo}", "a3")) == 2

    async def test_zadd_options_comparison(self, client, _s):
        await client.zadd("a{foo}", {"a1": 1})
        assert (
            int(await client.zadd("a{foo}", {"a3": 1}, comparison=PureToken.LT, change=True)) == 1
        )
        assert (
            int(await client.zadd("a{foo}", {"a3": 2}, comparison=PureToken.LT, change=True)) == 0
        )
        assert (
            int(await client.zadd("a{foo}", {"a3": 2}, comparison=PureToken.GT, change=True)) == 1
        )
        assert (
            int(await client.zadd("a{foo}", {"a3": 1}, comparison=PureToken.GT, change=True)) == 0
        )
    ...
    async def test_zintercard(self, client, _s):
        await client.zadd("a{foo}", {"a1": 1, "a2": 1, "a3": 1})
        await client.zadd("b{foo}", {"a3": 2, "a4": 2, "a5": 2})
    ...
    async def test_zmpop(self, client, _s):
        await client.zadd("a{foo}", {"a1": 1, "a2": 2, "a3": 3})
        await client.zadd("b{foo}", {"a1": 4, "a2": 5, "a3": 6})
    ...
    async def test_bzmpop(self, client, cloner, _s):
        ...
        async def _delayadd():
            ...
            async with clone:
                ...
                return await clone.zadd("a{foo}", {"a1": 42})
    ...
    async def test_zscan(self, client, _s):
        await client.zadd("a", {"a": 1, "b": 2, "c": 3})
