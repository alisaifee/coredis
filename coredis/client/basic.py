...
class Redis(Client[AnyStr]):
    ...
    def himport(
        ...
        """
        Return a :class:`~coredis.patterns.himport.HashImport` context manager for
        writing many hashes that share the same field names.

        :param fieldset: name identifying the field layout in Redis
        :param fields: field names, in order, matching the values passed to
         :meth:`~coredis.patterns.himport.HashImport.add`
        :return: A :class:`~coredis.patterns.himport.HashImport` context manager
        """
