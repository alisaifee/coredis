...
class PipelineCommandRequest(CommandRequest[CommandResponseT]):
    ...
    def __await__(self) -> Generator[None, None, CommandResponseT]:
        if hasattr(self, "response"):
            ...
            warnings.warn(
                """
Awaiting a pipeline command before executing the pipeline has no effect and 
is returning the pipeline instance itself for backward compatibility. 

If the objective is to just add a command to the pipeline call it normally without
the await. After executing the pipeline with ``execute()`` the return from the pipeline
command can be awaited to capture the result of the command if desired.
...
class Pipeline(Client[AnyStr], metaclass=PipelineMeta):
    ...
    async def _execute_transaction(
        ...
        for i, cmd in enumerate(commands):
            cmd.queued_response = requests[i]
