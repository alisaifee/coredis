...
class KeySpec:
    READONLY: ClassVar[
        ...
        b"XREAD": lambda args: (
            lambda kwpos: args[1 + kwpos : len(args) - (len(args) - (kwpos + 1)) // 2]
    ...
    ALL: ClassVar[Dict[bytes, Callable[[Tuple[ValueT, ...]], Tuple[ValueT, ...]]]] = {
        ...
        if b"STORE" in args
        else () + (lambda kwpos: (args[kwpos + 1],))(args.index(b"STOREDIST", 6))
        if b"STOREDIST" in args
        else (),
        b"GEORADIUSBYMEMBER": lambda args: (args[1],)
        + (lambda kwpos: (args[kwpos + 1],))(args.index(b"STORE", 5))
        if b"STORE" in args
        else () + (lambda kwpos: (args[kwpos + 1],))(args.index(b"STOREDIST", 5))
        if b"STOREDIST" in args
        else (),
        b"GEORADIUSBYMEMBER_RO": lambda args: (args[1],),
        b"GEORADIUS_RO": lambda args: (args[1],),
