                    ...
                    for arg in args:
                        if arg.get("type", None) == "pure-token":
                            tokens.append(
                                (
                                    sanitized(
                                        arg["name"].replace("-token", ""),
                                        ignore_reserved_words=True,
                                    ),
                                    arg["token"].upper(),
                                )
                            )
...
def sanitized(x, command=None, ignore_reserved_words=False):
    ...
    if not ignore_reserved_words and (
        cleansed_name in globals()["__builtins__"].__dict__ or keyword.iskeyword(cleansed_name)
    ):
...
def code_gen(ctx, debug: bool):
    ...
    if debug:
        ...
        os.system("REDIS_VERSION=8.10-rc2 docker-compose up redis-basic -d")
...
@code_gen.command()
@click.option("--path", default="docs/source/handbook/cluster_routing.rst")
@click.pass_context
def cluster_routing_doc(ctx, path: str):

    env = Environment()
    strategies = {}
    section_template_str = """

{% for strategy, groups in mapping.items() %}

====================={{'='*len(strategy)}}
Commands routed to {{strategy}}
====================={{'='*len(strategy)}}
...
{%- for command in commands %}
  :meth:`{{command["class"]}}.{{command["method"].__name__}}`
      The result is {{command["combine"]}}
...
{% endfor %}
    ...
    for obj in inspect.getmembers(coredis.commands._routing):
        if isinstance(obj[1], type) and issubclass(
            obj[1], coredis.commands._routing.RoutingStrategy
        ):
            if not inspect.isabstract(obj[1]):
                strategies[obj[1]] = []

    for name, method in inspect.getmembers(coredis.RedisCluster):
        if cmd := getattr(method, "__coredis_command", None):
            if cmd.cluster and cmd.cluster.routing_strategy:
                strategies[cmd.cluster.routing_strategy.__class__].append(
                    {"cmd": cmd, "method": method, "class": "coredis.RedisCluster"}
                )

    for class_name, item in inspect.getmembers(coredis.modules):
        if mod := getattr(item, "MODULE", None):
            for name, method in inspect.getmembers(item):
                if cmd := getattr(method, "__coredis_command", None):
                    if cmd.cluster and cmd.cluster.routing_strategy:
                        strategies[cmd.cluster.routing_strategy.__class__].append(
                            {"cmd": cmd, "method": method, "class": f"coredis.modules.{class_name}"}
                        )

    mapping = {}

    for strategy, commands in strategies.items():
        for command in commands:
            strategy_instance = command["cmd"].cluster.routing_strategy
            desc = mapping.setdefault(strategy_instance.description, {})
            desc.setdefault(command["cmd"].group.name, []).append(
                {
                    "method": command["method"],
                    "combine": strategy_instance.merge_callback.response_policy,
                    "class": command["class"],
                }
            )
    ...
    section_template = env.from_string(section_template_str)
    output = section_template.render()

    open(path, "w").write(output)
    print(f"Generated cluster routing doc at {path}")
...
def implementation(ctx, command, group, module, expr, debug=False):
    ...
    {%- endif -%}
    {%- endif -%})
    def {{method["name"]}}{{render_signature(method["rec_signature"], True, with_return=False)}} -> CommandRequest[Any]:
        \"\"\"
        {{method["summary"]}}
        ...
        {% if module %}
        return self.client.create_request(CommandName.{{sanitized(method["command"]["name"]).upper()}}, *command_arguments, callback=NoopCallback())
        {% else %}
        return self.create_request(CommandName.{{sanitized(method["command"]["name"]).upper()}}, *command_arguments, callback=NoopCallback())
        {% endif %}
"""
    env = Environment()
    env.globals.update(
    ...
    if group:
        ...
        if command or expr:
            rex = re.compile(expr) if expr else None
            commands = [
                k for k in commands if (rex.match(k["name"]) if rex else k["name"] in command)
            ]
    else:
