...
implementing the :class:`~coredis.patterns.cache.AbstractCache` interface and will:
...
3. If the cache returns a :data:`~coredis.patterns.cache.AbstractCache.confidence` value lower
   ...
   will be provided to the cache through a call to :meth:`~coredis.patterns.cache.AbstractCache.feedback` and
...
Specifically :class:`~coredis.patterns.cache.NodeTrackingCache` contains the implementation for a
single node and :class:`~coredis.patterns.cache.ClusterTrackingCache` tracks all the nodes in a redis cluster.

Users don't need to worry about how these implementations work, and instead can focus on implementing
a :class:`~coredis.patterns.cache.AbstractCache` instance or using the provided implementation, :class:`~coredis.patterns.cache.LRUCache`.
...
:class:`~coredis.patterns.cache.LRUCache` exposes a few configuration options to fine tune
the cache. Specifically the following constructor arguments might be of interest:

:paramref:`~coredis.patterns.cache.LRUCache.max_keys`
...
:paramref:`~coredis.patterns.cache.LRUCache.confidence`
...
:paramref:`~coredis.patterns.cache.LRUCache.dynamic_confidence`
